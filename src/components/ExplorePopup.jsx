...
const ExplorePopup = ({ open, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-start bg-black/60 backdrop-blur-lg">
      <div className="w-full max-w-2xl mt-16 flex flex-col items-center">
        <div className="w-full flex items-center mb-8 relative">
          <input
            ...
            autoFocus
          />
          <button className="absolute right-4 text-3xl text-[#38BDF8] font-bold" onClick={onClose}>&times;</button>
        </div>
        <div className="w-full max-h-[60vh] overflow-y-auto grid grid-cols-1 md:grid-cols-2 gap-8 pr-2">
          ...
          {filtered.map(perfume => (
            <div key={perfume.id} className="bg-[#1e293b] rounded-2xl p-6 flex flex-col items-center border border-[#38BDF8]/30 shadow-lg">
              ...
              <div className="text-[#C0C0C0] text-center mb-4 text-sm">{perfume.description}</div>
              <div className="flex gap-3 w-full">
                ...
                <button
                  ...
                  onClick={() => { dispatch({ type: "ADD_TO_CART", product: perfume }); onClose && onClose(); }}
                >
                  Buy Now
                </button>
              </div>
